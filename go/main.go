...
func initialize(c echo.Context) error {
	...
	for _, p := range paths {
		...
		cmdStrEstate := fmt.Sprintf("mysql -h %v -u %v -p%v -P %v %v < %v",
		...
		if err := exec.Command("bash", "-c", cmdStrEstate).Run(); err != nil {
			c.Logger().Errorf("Initialize script error : %v", err)
			return c.NoContent(http.StatusInternalServerError)
		}
...
func searchChairs(c echo.Context) error {
	...
	limitOffset := " ORDER BY popularity_v ASC, id ASC LIMIT ? OFFSET ?"
...
func searchRecommendedEstateWithChair(c echo.Context) error {
	...
	h := chair.Height
	d := chair.Depth
	query = `SELECT * FROM estate WHERE (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) OR (door_width >= ? AND door_height >= ?) ORDER BY popularity_v ASC, id ASC LIMIT ?`
	err = chairDb.Select(&estates, query, w, h, w, d, h, w, h, d, d, w, d, h, Limit)
	if err != nil {
		if err == sql.ErrNoRows {
			return c.JSON(http.StatusOK, EstateListResponse{[]Estate{}})
		}
...
func searchEstateNazotte(c echo.Context) error {
	...
	// TOOD おもそう
	query := `SELECT * FROM estate WHERE latitude <= ? AND latitude >= ? AND longitude <= ? AND longitude >= ? ORDER BY popularity_v ASC, id ASC`
	err = estateDb.Select(&estatesInBoundingBox, query, b.BottomRightCorner.Latitude, b.TopLeftCorner.Latitude, b.BottomRightCorner.Longitude, b.TopLeftCorner.Longitude)
	if err == sql.ErrNoRows {
